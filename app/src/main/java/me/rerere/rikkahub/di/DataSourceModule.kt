...
val dataSourceModule = module {
            ...
            .addCallback(object : RoomDatabase.Callback() {
                override fun onOpen(db: SupportSQLiteDatabase) {
                    runCatching {
                        ...
                        android.util.Log.e("DataSourceModule", "FTS/jieba init failed; continuing without it", it)
                    }
                }
            })

        if (libSimpleSo.exists()) {
            ...
            android.util.Log.w(
                "DataSourceModule",
                "libsimple.so missing at ${libSimpleSo.absolutePath}; using default SQLite helper",
            )
        }
    ...
    single {
        ConversationCompressHelper(providerManager = get())
    }

    single {
        SessionOverviewHelper(
            compressHelper = get(),
